...
Rails.application.config.sorcery.configure do |config|
  ...
  # config.twitter.key = ENV["SPE_TWITTER_ID_TEST"]
  # config.twitter.secret = ENV["SPE_TWITTER_SECRET_TEST"]
  # config.twitter.callback_url = "http://0.0.0.0:3000/oauth/callback?provider=twitter"
  # config.twitter.user_info_mapping = {:email => "screen_name"}
 
  config.twitter.key = ENV["SPE_TWITTER_ID"]
  config.twitter.secret = ENV["SPE_TWITTER_SECRET"]
  config.twitter.callback_url = "http://sublime-packages-evaluator.herokuapp.com/oauth/callback?provider=twitter"
  config.twitter.user_info_mapping = {:email => "screen_name"}
  
  # config.facebook.key = ENV["SPE_FACEBOOK_ID_TEST"]
  # config.facebook.secret = ENV["SPE_FACEBOOK_SECRET_TEST"]
  # config.facebook.callback_url = "http://0.0.0.0:3000/oauth/callback?provider=facebook"  
  # config.facebook.display = "popup"
  # config.facebook.user_info_mapping = {:email => "name"}
  # config.facebook.access_permissions = ["email", "publish_stream"]

  config.facebook.key = ENV["SPE_FACEBOOK_ID"]
  config.facebook.secret = ENV["SPE_FACEBOOK_SECRET"]
